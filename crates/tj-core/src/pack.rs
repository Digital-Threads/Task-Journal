...
mod tests {
    ...
    fn is_noise_flags_machine_markers_not_real_reasoning() {
        ...
        assert!(is_noise("Conversation compacted at 2026-06-07T11:47:33Z; preceding events…"));
        assert!(is_noise("  [Conversation compacted]"));
        assert!(is_noise(
            "Caveat: The messages below were generated by the user while running local commands."
        ));
        // real reasoning is kept
        assert!(!is_noise("Use axum for the server because it has better middleware."));
        assert!(!is_noise("Rejected: per-stage sessions lose accumulated context."));
    }
